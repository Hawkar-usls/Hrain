(function (root, factory) {
  ...
  if (root) root.JANUSDemiHeadHRainApply = api;
})(typeof globalThis !== 'undefined' ? globalThis : this, function (root) {
  'use strict';

  const ENVELOPE_TYPE = 'JANUS_DEMIHEAD_LOCAL_PROPOSAL_V1';
  const PROPOSAL_SCHEMA = 'janus.demihead.local_proposal.v1';
  const TARGET_HEMISPHERE = 'LEFT_HRAIN';
  const TARGET_REPOSITORY = 'Hawkar-usls/Hrain';
  const STORAGE_KEY = 'hrain_v10_4_restore';
  ...
  async function prepareAcceptedMutation(workspace, normalizedGraph, envelope) {
    ...
    next.nodes.push({
      ...
      chatHistory: [],
      origin: 'SYSTEM',
      demiheadProposalId: proposal.proposal_id,
      demiheadProposalSha256: envelope.proposal_sha256
    });
    return {
  ...
  function buildReceipt({proposalId, proposalSha256, beforeGraphSha256, afterGraphSha256, nodeId}) {
    return {
